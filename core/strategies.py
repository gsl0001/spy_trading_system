...
class BacktestEngine:
    ...
    def _simulate_trades(self, entries, exits_logic, type='long', fixed_stop_dist=0.03, use_ml=False, return_signal=False, return_logic=False, strat_name=""):
        ...
        # Pre-extract arrays for blazing fast iteration
        ...
        if use_ml and self.ml_filter and self.ml_filter.is_trained:
            ml_feats = df[self.ml_filter.features].values
            ml_valid = ~np.isnan(ml_feats).any(axis=1)
        ...
        for i in range(len(df)):
            if not in_position:
                if sig_arr[i]:
                    ...
                    if use_ml and self.ml_filter and self.ml_filter.is_trained:
                        if ml_valid[i]:
                            prob = self.ml_filter.predict(ml_feats[i])
                ...
                if type == 'long':
                    hwm = max(hwm, high_arr[i])
                else:
                    hwm = min(hwm, low_arr[i])
                ...
                if not exit_signal:
                    ...
                    if self.global_stop_loss > 0:
                        ...
                        if (type == 'long' and low_arr[i] <= sl_price) or (type == 'short' and high_arr[i] >= sl_price):
                    ...
                    if not exit_signal and self.global_take_profit > 0:
                        ...
                        if (type == 'long' and high_arr[i] >= tp_price) or (type == 'short' and low_arr[i] <= tp_price):
                    ...
                    if not exit_signal and self.trailing_stop > 0:
                        ...
                        if (type == 'long' and low_arr[i] <= ts_price) or (type == 'short' and high_arr[i] >= ts_price):
                            exit_signal, exit_price = True, ts_price
                            
                    # Global Time Stop
                    if not exit_signal and self.max_hold_bars > 0 and bars_in >= self.max_hold_bars:
                        exit_signal, exit_price = True, close_arr[i]
    ...
    def strategy_36(self, use_ml=False, **kwargs):
        ...
        if not self.ml_filter or not self.ml_filter.is_trained:
            ...
            signal_df = self.get_all_signals([f"Strategy {i}" for i in range(1, 38)])
            consensus = signal_df.mean(axis=1)
            entries = consensus > 0.4 # Need 40% agreement
        else:
            # Full AI Weighting
            signal_names = [f"Strategy {i}" for i in range(1, 38)]
    ...
    def strategy_37(self, use_ml=False, **kwargs):
        """Strategy 37: VWAP-Keltner Volatility Compression Breakout
        
        Thesis: Institutional accumulation creates a fingerprint — price compresses 
        near VWAP while flow shifts (CMF rises). When the Keltner squeeze releases 
        with MACD momentum confirmation, it signals a high-probability breakout.
        
        Entry (ALL must be true):
          1. Price within 0.5% of VWAP_Proxy (institutional fair value zone)
          2. Bollinger Bands inside Keltner Channels (volatility squeeze)
          3. CMF > 0.05 (net institutional buying pressure)
          4. MACD histogram crosses above zero (momentum ignition)
        ...
        kc_upper = kc_mid + (df['ATR_14'] * 1.5)
        kc_lower = kc_mid - (df['ATR_14'] * 1.5)
        
        # --- Entry Conditions ---
        # 1. VWAP Proximity: price within 0.5% of VWAP_Proxy
        vwap_proximity = (df['Close'] / df['VWAP_Proxy'] - 1).abs() < 0.005
        
        # 2. Keltner Squeeze: Bollinger Bands compressed inside Keltner Channels
        squeeze_on = (df['BB_Upper'] < kc_upper) & (df['BB_Lower'] > kc_lower)
        # We want the squeeze to RELEASE (was on, now off) — breakout moment
        squeeze_release = (~squeeze_on) & (squeeze_on.shift(1))
        
        # 3. Institutional Flow: CMF > 0.05 (net buying pressure)
        flow_positive = df['CMF'] > 0.05
        
        # 4. Momentum Ignition: MACD histogram crosses above zero
        macd_cross_up = (df['MACD_Hist_Dist'] > 0) & (df['MACD_Hist_Dist'].shift(1) <= 0)
        
        # Combined entry: all four conditions active within a 2-bar window
        # (squeeze_release OR vwap_proximity) to allow slight timing offsets
        entries = (vwap_proximity | squeeze_release) & flow_positive & macd_cross_up
        
        def exit_logic(df, i, ep, ei):
            atr_at_entry = df['ATR_14'].iloc[ei]
            target = ep + (2.0 * atr_at_entry)
            stop = ep - (1.5 * atr_at_entry)
            bars_in = i - ei
            ...
            # Time stop
            if bars_in >= 8:
                return True, df['Close'].iloc[i]
            return False, 0
        
        return self._simulate_trades(entries, exit_logic, fixed_stop_dist=0.03, use_ml=use_ml, strat_name="Strategy 37", **kwargs)
    ...
    def run_ai_selective_master(self, strategy_options):
        ...
        logics = {}
        for s in strategy_names:
            strat_key = s.split(':')[0].strip()
            logics[strat_key] = self.run_strategy(s, return_logic=True)
        ...
        # Extreme Pandas Optimizations
        close_arr = df['Close'].values
        sig_arr = signals_df.values
        sig_cols = signals_df.columns
        
        e_feats = getattr(self.ml_filter, 'ensemble_features', [])
        if e_feats and self.ml_filter and getattr(self.ml_filter, 'is_ensemble_trained', False):
            full_feat_df = pd.concat([df[self.ml_filter.base_features], signals_df], axis=1).fillna(0)
            model_input_arr = full_feat_df.reindex(columns=e_feats).fillna(0).values
        else:
            model_input_arr = None
            
        for i in range(len(df)):
            # Track collisions (bars with >1 signal)
            firing_idx = np.where(sig_arr[i])[0]
            firing_count = len(firing_idx)
            if firing_count > 1:
                total_collisions += 1

            if not in_position:
                if firing_count > 0 and self.ml_filter and self.ml_filter.is_trained:
                    # Pick the one with highest trust score
                    best_strat = None
                    max_trust = -1
                    
                    firing_cols = [sig_cols[idx] for idx in firing_idx]
                    
                    if model_input_arr is not None:
                        prob = self.ml_filter.predict(model_input_arr[i])
